...
package it.unipi.thesis.andrea.esposito.onjag.util

import org.apache.spark.rdd.RDD
import it.unipi.thesis.andrea.esposito.onjag.protocols.{JaBeJaVertexContext, FindSubPartitionProtocol}
import it.unipi.thesis.andrea.esposito.onjag.core.{Vertex, OnJag, PersistenceMode}
 ...
 */
object ApproximateNormalizedMinCut {

  var maxSupersteps = -1

  def measureFromJaBeJa[K: ClassTag](sc: SparkContext,
    ...
    val graph = jabejaResult.map(e => {
      val jabejaCxt = e._2.protocolsContext(jabejaIndex).asInstanceOf[JaBeJaVertexContext[K]]
      (e._1, jabejaCxt.neighbourView.map(v => v._1), jabejaCxt.color)
    })
    measure[K](sc, graph, colors, alfa, x, y, z, computeDistances, distancesSamplingPercentage, numPartition, storageLevel, persistenceMode)
  }

  /**
   * Compute the NQcut measure over a given graph
   */
  def measure[K: ClassTag](sc: SparkContext,
    ...
    onjag.setMaxStep(maxSupersteps)
    ...
    val result = onjag.run(sc, verts, messages, actualNumPartitions, persistenceMode)
