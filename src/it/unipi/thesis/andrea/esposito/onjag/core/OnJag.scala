 ...
 * Comment/Uncomment to activate/deactivate completely the Misfire MODE !!!!!
 */
...
object OnJag {
  ...
  val PERSISTENCE_MODE_DEFAULT = PersistenceMode.DELETE_IMMEDIATELY_SYNCHED
...
class OnJag(@transient sc: SparkContext, storageLevel: StorageLevel, protocols_param: Protocol*) extends Logging with Serializable {
   ...
   */
  def run[K: ClassTag, V <: Vertex[K] : ClassTag, A: ClassTag](sc: SparkContext,
                                                               vertices: RDD[(K, V)],
    ...
    do {
      ...
      if (maxStep < 0 || superstep <= maxStep) {
        ...
        for (i <- 0 until protocols.length) {
          if (superstep >= protocols(i).startStep) {
            val executable = arrStepsCounter(i) <= 0
            if (executable) {
              ...
              protocols(i).beforeSuperstep(sc, superstep_)
              ...
              protocols(i).afterSuperstep(sc, superstep_)
              tmpProtocolBroadcast = sc.broadcast(protocols(i)) // Re-broadcast to update broadcast values
              protocols_broadcast(i).unpersist()
              protocols_broadcast(i) = tmpProtocolBroadcast
              ...
              arrStepsCounter(i) = protocols(i).step
            } else {
  ...
  def run[K: ClassTag, V <: Vertex[K] : ClassTag, A: ClassTag](
    ...
    run(sc, vertices, messages, combiner, aggregator, checkpointConditions, new HashPartitioner(numPartitions), persistenceMode)
  }
  ...
  def run[K: ClassTag, V <: Vertex[K] : ClassTag, A: ClassTag](
    ...
    run(sc, vertices, messages, combiner, aggregator, checkpointConditions, numPartitions, OnJag.PERSISTENCE_MODE_DEFAULT)
  }
  ...
  def run[K: ClassTag, V <: Vertex[K] : ClassTag](
    ...
    run(sc, vertices, messages, combiner, aggregator, checkpointConditions, numPartitions, persistenceMode)
  }
  ...
  def run[K: ClassTag, V <: Vertex[K] : ClassTag](
    ...
    run(sc, vertices, messages, checkpointConditions, numPartitions, OnJag.PERSISTENCE_MODE_DEFAULT)
  }
  ...
  def run[K: ClassTag, V <: Vertex[K] : ClassTag](
    ...
    run(sc, vertices, messages, checkpointConditions, numPartitions, persistenceMode)
  }
  ...
  def run[K: ClassTag, V <: Vertex[K] : ClassTag](
    ...
    run(sc, vertices, messages, numPartitions, OnJag.PERSISTENCE_MODE_DEFAULT)
  }
